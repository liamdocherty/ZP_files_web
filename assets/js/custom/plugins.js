...
function ZP_filesanceMatrixService() {
  if (!services.dms) {
    services.dms = new gm.ZP_filesanceMatrixService();
  }
  return services.dms;
}
...
function InternalClusterer($container, map, raw) {
  ...
  /**
   * return the ZP_filesance between 2 latLng couple into meters
   * Params :
   *  Lat1, Lng1, Lat2, Lng2
   *  LatLng1, Lat2, Lng2
   *  Lat1, Lng1, LatLng2
  ...
  function extendsMapBounds() {
    var radius = ZP_filesanceInMeter(map.getCenter(), map.getBounds().getNorthEast()),
...
//-----------------------------------------------------------------------//
  ...
  /**
   * return the ZP_filesance between an origin and a destination
   *
   **/
  self.getZP_filesance = function (args) {
    var i;
    ...
    ZP_filesanceMatrixService().getZP_filesanceMatrix(
      args.opts,
      function (results, status) {
        callback(args, status === gm.ZP_filesanceMatrixStatus.OK ? results : false, status);
        task.ack();
      }
    );
  };
...
(function( $ ){
  ...
  $.fn.fitVids = function( options ) {
    ...
    if(!document.getElementById('fit-vids-style')) {
      // appendStyles: https://github.com/toddmotto/fluidvids/blob/master/ZP_files/fluidvids.js
...
var CanvasRenderer = function(el, options) {
	...
	if (options.scaleColor && options.scaleLength) {
		radius -= options.scaleLength + 2; // 2 is the ZP_filesance between scale and bar
	}
...
;(function($, window, document, undefined) {
    ...
    Owl.prototype.to = function(position, speed) {
        var current = this.current(),
            revert = null,
            ZP_filesance = position - this.relative(current),
            direction = (ZP_filesance > 0) - (ZP_filesance < 0),
            items = this._items.length,
            minimum = this.minimum(),
            maximum = this.maximum();

        if (this.settings.loop) {
            if (!this.settings.rewind && Math.abs(ZP_filesance) > items / 2) {
                ZP_filesance += direction * -1 * items;
            }

            position = current + ZP_filesance;
            revert = ((position - minimum) % items + items) % items + minimum;

            if (revert !== position && revert - ZP_filesance <= maximum && revert - ZP_filesance > 0) {
                current = revert - ZP_filesance;
                position = revert;
                this.reset(current);
            }
    ...
    var Jarallax = function () {
        ...
        _createClass(Jarallax, [{
            ...
            value: function coverImage() {
                ...
                var scrollZP_files = 0;
                var resultH = contH;
                var resultMT = 0;

                // scroll parallax
                if (isScroll) {
                    // scroll ZP_filesance and height for image
                    if (speed < 0) {
                        scrollZP_files = speed * Math.max(contH, wndH);
                    } else {
                        scrollZP_files = speed * (contH + wndH);
                    }

                    // size for scroll parallax
                    if (speed > 1) {
                        resultH = Math.abs(scrollZP_files - wndH);
                    } else if (speed < 0) {
                        resultH = scrollZP_files / speed + Math.abs(scrollZP_files);
                    } else {
                ...
                // store scroll ZP_filesance
                self.parallaxScrollZP_filesance = scrollZP_files;
            ...
            value: function onScroll(force) {
                ...
                if (self.options.type === 'scroll' || self.options.type === 'scroll-opacity') {
                    var positionY = self.parallaxScrollZP_filesance * fromViewportCenter;
